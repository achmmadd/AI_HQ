...
export function FumeroEmbedFrame({
  ...
  if (!code?.trim() || !srcDoc.trim()) {
    return (
      <div className="flex h-dvh w-full flex-col items-center justify-center gap-2 bg-[#FAFAFA] p-8 text-center">
        <p className="text-sm font-medium text-[#171717]">Geen preview beschikbaar</p>
        <p className="text-xs text-[#737373]">App-code ontbreekt of is ongeldig.</p>
      </div>
    );
  }
  ...
  if (loadFailed) {
    return (
      <div className="flex h-dvh w-full flex-col items-center justify-center gap-3 bg-[#FAFAFA] p-8 text-center">
