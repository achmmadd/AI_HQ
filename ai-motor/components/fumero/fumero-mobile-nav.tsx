...
const ITEMS = [
  ...
  { href: "/fumero/automations", label: "Automatisering", icon: Workflow, match: "/fumero/automations" },
...
function isActive(pathname: string, match: string): boolean {
  ...
  if (match === "/fumero/settings") {
    return pathname.startsWith("/fumero/settings") || pathname === "/fumero/pay";
  }
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex min-h-[52px] min-w-[3.25rem] flex-col items-center justify-center gap-0.5 px-1 py-2 text-[10px] font-medium transition-colors",
                active
