...
const NAV_BY_WORKSPACE: Record<WorkspaceId, NavItem[]> = {
  ...
  personal: [
    { href: "/chat", label: "Motor Chat", icon: Bot },
...
export function MotorsSidebar() {
  ...
  useEffect(() => {
    void fetch("/api/bookkeeping/health", { credentials: "include" })
      .then((r) => r.json())
      .then((j: { pending_approvals?: number }) =>
        setInboxCount(j.pending_approvals ?? 0)
      )
  ...
  return (
    <aside
      style={{ width }}
      className="fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-border/60 bg-surface/92 pt-[env(safe-area-inset-top)] font-ws backdrop-blur-2xl transition-[width] duration-200 ease-out supports-[backdrop-filter]:bg-surface/75"
    >
      {collapsed ? (
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto overscroll-contain px-2 pb-2">
        {nav.map(({ href, label, icon: Icon }) => {
          ...
          const active =
            ...
            (href === "/bokas" && pathname === "/bokas");
          const showBadge =
            href === "/bokas/bonnen" && inboxCount != null && inboxCount > 0;
          return (
            <Link
              key={href}
              href={href}
