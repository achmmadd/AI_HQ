...
export function validateAppCode(code: string): ValidateAppCodeResult {
  ...
  if (/[^"'`]\*\*[^"'`]/.test(c)) {
    return {
      valid: false,
      error:
        "Gebruik Math.pow() ipv ** operator voor compatibiliteit",
    };
  }
  ...
  if (!/<!DOCTYPE/i.test(c) && !/<html[\s>]/i.test(c)) {
    return { valid: false, error: "Ontbrekend HTML-document (<!DOCTYPE of <html>)" };
  }
  ...
  if (!/<script[\s>]/i.test(c)) {
    return { valid: false, error: "Geen <script>; gebruik vanilla JS in één script-blok" };
  }

  if (/type\s*=\s*["']module["']/i.test(c)) {
    return { valid: false, error: "Geen type=module; gebruik één plain <script> zonder modules" };
  }
