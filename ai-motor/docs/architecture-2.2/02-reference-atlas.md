...
| Orchestration | Inngest/DBOS | Durable step functions in TypeScript over bestaande Postgres | E2/E4 | SoundCloud, Resend (Inngest); BMS, Dosu (DBOS) | Standalone worker op Hetzner 16 GB, naast PG | Inngest-store vereist eigen backup; DBOS heeft dunnere UI | Configure: bevestigingsspike ADR-101, exact één engine |
...
| **Sandboxing** | Codex | Container per taak; two-phase (setup mét internet/secrets → agent zónder, domain-allowlist, method-restricties) | E2 | ChatGPT-schaal | Exfiltratie-preventie | Complexiteit | Patroon behouden; sandbox-vloot pas na AM-1-gate |
...
| **Code review** | Claude Code | Adversarial review met verse context: reviewer ziet alleen diff+criteria | E2 | Breed | Geen self-grading-bias | Reviewer vindt áltijd iets → scope-instructie nodig | Patroon behouden; aparte adversarial stap bevroren door AM-1 |
...
| Observability | Langfuse | LLM-tracing SaaS/OSS | E2/E4 | Breed | Direct bruikbaar | EU-regio+DPA nodig; PII-content verboden | Configure met tracing per dataklasse (AM-4) |
...
| **Mobile command center** | Devin | Slack/Linear/Jira als volwaardige intake+review-kanalen | E2 | Enterprise | Werk waar de eigenaar is | — | Configure: Telegram alleen notificatie+link; inhoud/approval in Motor UI |
...
| **Productization** | Coze Studio | Compile/runtime-scheiding voor workflows; DDD-lagen voor multi-tenant platform | E4 (Apache-2.0, jul 2025) | ByteDance-schaal (E3) | Bewezen platformstructuur | Zwaar voor één technicus | Watchlist; eerst één hands-on pilotklant zonder SLA |
...
**Wat níet af is (de echte gaps, → [fit-gap](04-fit-gap-en-adoptieladder.md)):** de credential-broker Action Gateway, Motor Kernel/reconciliation en business-brede evidence-standaard. Een Playbook-registry is bewust bevroren; git-map + statuskolom volstaat.
...
| A2 | Playbook-format (outcome, input, procedure, postconditions, forbidden, foutafhandeling, approvals, bewijs) | Devin | Git-map + statuskolom §21; registry bevroren door AM-1 |
...
| A10 | Adversarial review met verse context (diff + criteria only) | Claude Code / Anthropic | Patroon bewaard; aparte stap bevroren tot AM-1-gate |
