...
| Onderdeel | Locatie |
|-----------|---------|
| Workflow (direct Dify; geen Optimus/CEO-router) | `factory-os/systeem/n8n-workflows/factory_os_dispatcher.json` |
...
**Gedrag:** Alleen **Dify** (all-in-one agent): `Prepare` checkt de app-key, daarna **één** `POST …/v1/chat-messages` met `inputs.klant` en `query` (streaming/SSE). **Geen** Optimus/OpenClaw-router meer in deze workflow. Vereist `DIFY_AGENT_API_KEY` **of** `DIFY_RESEARCH_API_KEY`.
